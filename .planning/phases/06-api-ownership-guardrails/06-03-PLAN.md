...
must_haves:
  ...
  key_links:
    ...
    - from: 'pages/api/fueling/index.ts'
      ...
      via: '401/403 guardrail envelopes'
      pattern: 'sendUnauthenticated'
    - from: 'pages/api/fueling/[id].ts'
      to: 'pages/api/_shared/errors.ts'
      via: 'NOT_FOUND cloak for non-owner GET'
      pattern: 'sendForbiddenAsNotFound'
    - from: 'hooks/useUpdateFueling.ts'
      to: 'pages/api/fueling/[id].ts'
      via: 'reads standardized error envelope message'
      pattern: 'error\.message'
---
...
<verify>
Open `pages/api/fueling/index.ts` and confirm:

- `requireSessionUserId` gates both methods
- `GET` query filters through `vehicle.user_id = sessionUserId`
- `POST` verifies ownership with `ensureOwnedVehicle` before any create/update
- write operations are wrapped in a Prisma transaction when mileage updates occur
  </verify>
  <done>Fueling list/create is owner-scoped and rejects cross-owner create attempts without side effects.</done>
  </task>
...
<verify>
Open `pages/api/fueling/[id].ts` and confirm:

- `requireSessionUserId` runs before ownership checks
- `ensureOwnedFueling` is called before any read/write
- non-owner `GET` uses NOT_FOUND envelope with 403 status
- non-owner `PUT`/`DELETE` use FORBIDDEN envelope and do not call update/delete paths
  </verify>
  <done>Fueling detail endpoint blocks cross-user access, with GET cloaked as NOT_FOUND and writes blocked before mutations.</done>
  </task>
...
<verify>
Open each hook file and confirm:

- error parsing attempts `error.message` from a JSON response body
- fallback message remains in place when parsing fails
- success toast + query invalidation logic are unchanged
  </verify>
  <done>Fueling mutation hooks surface server envelope messages without altering existing success flows.</done>
  </task>
