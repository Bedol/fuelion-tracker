...
Create `pages/api/_shared/auth.ts` with `requireSessionUserId(req, res)` using `getServerSession(req, res, authOptions)`. Return numeric `userId` when authenticated; otherwise write the shared `401` envelope and return `null`.
</action>
<verify>TypeScript compiles helpers without `any`; helper exports are importable from API routes.</verify>
<done>Shared auth/error helper APIs exist and encode the locked 401/403 behavior contract.</done>
</task>
...
Return only safe data needed by callers (ids and relation ids), not full records. Include helpers that support list filtering and cross-owner POST guard checks (e.g., vehicle ownership existence for fueling create).

Do not mutate data in these helpers. They are read-only guards to preserve strict no-op guarantees on failed writes.
</action>
<verify>Guard helpers return null/defined results correctly when called with owned vs foreign ids in route-level dry checks.</verify>
<done>Reusable ownership checks exist for both vehicle and fueling resources and can be called before any write query.</done>
</task>
...
<verify>Run `npm run build` and confirm no type/import errors from new helper modules.</verify>
